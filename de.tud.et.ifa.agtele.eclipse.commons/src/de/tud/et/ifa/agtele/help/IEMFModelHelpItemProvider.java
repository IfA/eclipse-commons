...
/**
 * This Interface is used to render HTMl help text from an {@link EObject}. Use
 * the Methods {@link #getHelpItemDescription(EObject)} to render the default
 * help view.
 *
 * The generation of the HTML document containing the help relies on html
 * snippets containing placeholdes like '%PLACEHOLDER%' and render methods
 * replacing the placeholders with data from the {@link HelpItemData} objects
 * and the references {@link EObject}s such as {@link EClass},
 * {@link EReference}, etc.
 *
 * The {@link HelpItemData} and {@link HelpItemDescription} objects are created
 * using an overridable {@link HelpItemFactory}.
 *
 * The rendering can be adapted by:
 * <ul>
 * <li>overriding the getter methods for the general html templates (change the
 * logical structure of the generated document):
 * <ul>
 ...
 * </li>
 *
 * <li>overriding the methods for creating the default HTML utility (changes
 * style and behavior)
 * <ul>
 ...
 * </li>
 *
 * <li>overriding the getter methods for the EMF specific html templates (change
 * the logical structure of the help item display):
 * <ul>
 * <li>{@link #getAttributeTemplate(boolean)}</li>
 * <li>{@link #getEEnumLiteralTemplate()}</li>
 * <li>{@link #getNCReferenceeTemplate()},
 * {@link #getNCReferenceTemplate(boolean)}</li>
 * <li>{@link #getCReferenceTemplate(boolean)}</li>
 * <li>{@link #getCReferenceChildTemplate(boolean)}</li>
 * </ul>
 * </li>
 * <li>overriding the render methods, that process the {@link HelpItemData} and
 * replace the placeholders such as '%PLACEHOLDER%' from the html templates with
 * the EMF specific content:
 * <ul>
 ...
 * </ul>
 * </li>
 * <li>providing a custom {@link HelpItemFactory} implementation to the
 * {@link #getHelpItemDescription(EObject, HelpItemFactory)} method that
 * generates the {@link HelpItemDescription} including the {@link HelpItemData}
 * objects that are consumed by the render-methods.</li>
 * </ul>
 *
 * @author cmartin
 * @author Baron
...
public interface IEMFModelHelpItemProvider {
	 ...
	 *
	 * @return Contains the following placeholders: %TITLE%, %HEAD%, %STYLE%,
	 *         %BODY%, %SCRIPT%.
	 */
	public default String getHTMLTemplate() {
		return "<!DOCTYPE html>" + "<html lang=\"en\">" + "<head>" + "<meta charset=\"UTF-8\">"
				+ "<title>%TITLE%</title>" + "%HEAD%" + "%STYLE%" + "</head>" + "<body>" + "%BODY%" + "%SCRIPT%"
				+ "</body>" + "</html>";
	}
	...
	/**
	 * The template for a tag container that is to be displayed for some of the
	 * features.
	 *
	...
	/**
	 * The template for a tag, that is to be displayed inside the
	 * {@link #getTagContainerTemplate()}.
	 *
	...
	public default String render(HelpItemDescription helpItemDescription) {
		...
		String result = this.getHTMLTemplate().replace("%TITLE%", eClass.getName()).replace("%STYLE%", this.getStyles())
				.replace("%BODY%", this.renderBody(helpItemDescription)).replace("%SCRIPT%", this.getScripts());
	 ...
	 *
	 * @return Calls {@link #getCSSTemplate()} and replaces the %RULES%
	 *         placeholder.
	 */
	public default String getStyles() {
		...
		styles += this.getCSSTemplate().replace("%RULES%",
				// ".heading:not(h1), .description, .category, .sub-category,
				// .tag-container {"
				// + "padding-left: 20px;"
				// + "}"
				"* {" + "font-size:16px;" + "background-color:rgb(240,240,240);" + "color:rgb(40,40,40);" + "}"
						+ "body {" + "overflow-y:scroll;" + "}" + ".tag {" + "display:inline-block;"
						+ "border: 1px solid black;" + "border-radius: 2px;" + "}" + "h1,h2,h3,h4,h5 {" + "margin:0;"
						+ "}" + ".tag{" + "padding-left: 5px;" + "padding-right: 5px;" + "}" + ".category{"
						+ "padding-left:10px;" + "}" + ".description{" + "padding-left:20px;" + "margin-top:3px;"
						+ "margin-bottom:6px;" + "}" + "h1 {" + "font-size:24px;" + "padding-top: 5px;"
						+ "padding-bottom:5px;" + "padding-left:15px;" + "background-color:rgb(200,200,200);" + "}"
						+ "h1 + div {"
						// + "padding-left: 30px;"
						+ "}" + "h2 {" + "font-size:22px;" + "padding-top: 4px;" + "padding-bottom:4px;"
						+ "padding-left:15px;" + "background-color:rgb(210,210,210);" + "}" + "h2 + div {"
						// + "padding-left:30px;"
						+ "}" + "h3 {" + "font-size:20px;" + "padding-top: 3px;" + "padding-bottom:3px;"
						+ "padding-left:15px;" + "background-color:rgb(220,220,220);" + "}" + "h3 + div {"
						// + "padding-left:35px;"
						+ "}" + "h4 {" + "text-decoration:underline;" + "font-weight:normal;" + "padding-top: 3px;"
						+ "padding-bottom:3px;" + "padding-left:25px;"
						// + "background-color:rgb(230,230,230);"
						+ "}" + "h4 + div {"
						// + "padding-left:40px;"
						+ "}" + "h5 {" + "padding-bottom: 1px;" + "padding-top: 3px;" + "padding-bottom: 3px;"
						+ "padding-left:35px;" + "background-color:rgb(240,240,240);" + "}" + "h5+div {"
						// + "padding-top:5px;"
						+ "padding-bottom:5px;" + "padding-left:45px !important;" + "margin:0;" + "}" + ".expandable {"
						+ "cursor:pointer;" + "}" + ".expandable.collapsed+.sub-category {" + "display:none;" + "}"
						+ ".expandable.collapsed:before{" + "content: '+ ';" + "}" + ".expandable.expanded:before{"
						+ "content: '- ';" + "}" + "");
	...
	/**
	 * Delivers the default scripts for the generated help. Calls
	 * {@link #getJavaScriptTemplate()} and replaces the %CODE% placeholder.
	 *
	 * @return the jquery script and the custom script for collapsing and
	 *         expanding the help texts.
	 */
	public default String getScripts() {
		...
		try {
			scripts += this.getJavaScriptTemplate().replace("%CODE%", BundleContentHelper
					.getBundleFileString("de.tud.et.ifa.agtele.eclipse.commons", "files/jquery.min.js"));
		} catch (IOException e) {
			success = false;
			e.printStackTrace();
		}
		if (success) {
			scripts += "\n" + this.getJavaScriptTemplate().replace("%CODE%",
					"" + "(function(){" + "$('.expandable').on('click', function () {"
							+ "if($(this).is('.collapsed')) {"
							+ "$(this).removeClass('collapsed').addClass('expanded');" + "} else {"
	...
	/**
	 * The logical structure of the help document. Contains the placeholders
	 * %TITLE%, %DESCRIPTION%, [%ATTRIBUTES%, %NCREFERENCES%, %CREFERENCES%].
	 *
	 * @param includeAttributes
	 *            Whether to include the attributes section
	 * @param includeNCReferences
	 *            Whether to include the non-containment references section
	 * @param includeCReferences
	 *            Whether to include the containment references section
	 * @return The prepared template.
	 */
	public default String getBodyTemplate(boolean includeAttributes, boolean includeNCReferences,
			boolean includeCReferences) {
		return "<h1 class=\"heading class-heading\">%TITLE% - Help</h1>"
				+ "<div class=\"class-description description\">%DESCRIPTION%</div>"
				+ (includeAttributes ? "<h2 class=\"heading attributes-heading\">Attributes</h2>"
						+ "<div class=\"class-attributes category\">%ATTRIBUTES%</div>" : "")
				+ (includeNCReferences ? "<h2 class=\"heading ncreferences-heading\">Non-Containment References</h2>"
						+ "<div class=\"class-nc-references category\">%NCREFERENCES%</div>" : "")
				+ (includeCReferences ? "<h2 class=\"heading creferences-heading\">Containment References</h2>"
						+ "<div class=\"class-c-references category\">%CREFERENCES%</div>" : "");
	}

	/**
	 * Calls {@link #getBodyTemplate(boolean, boolean, boolean)} and replaces
	 * its placeholders by invoking {@link #renderAttributes(List)},
	 * {@link #renderNCReferences(List)}, {@link #renderCReferences(List)}
	 *
	 * @param helpItemDescription
	...
	public default String renderBody(HelpItemDescription helpItemDescription) {
		...
		return this.getBodyTemplate(!attributes.isEmpty(), !ncRefs.isEmpty(), !cRefs.isEmpty())
				.replace("%TITLE%", eClass.getName()).replace("%DESCRIPTION%", eClass.getDocumentation())
				.replace("%ATTRIBUTES%", this.renderAttributes(attributes))
				.replace("%NCREFERENCES%", this.renderNCReferences(ncRefs))
				.replace("%CREFERENCES%", this.renderCReferences(cRefs));
	}

	/**
	 * The default html template for the display of an attribute help text.
	 * Contains the placeholders %NAME%, %TYPE%, %DESCRIPTION%,
	 * [%EENUMDESCRIPTION%, %EENUMVALUES%]
	 *
	 * @param isEEnum
	 *            Whether to include the sections for documenting an EEnum.
	...
	public default String getAttributeTemplate(boolean isEEnum) {
		return "<h3 class=\"heading attribute-heading\">%NAME% \u00bb %TYPE% " + (isEEnum ? "(EEnum)" : "") + "</h3>"
				...
				+ (isEEnum ? "</br>" + "%EENUMDESCRIPTION%" : "")

				+ "</div>"
				+ (isEEnum ? "<h4 class=\"heading attribute-values-heading expandable collapsed\">Possible Values</h4>"
						+ "<div class=\"eenum-values sub-category\">%EENUMVALUES%</div>" : "");
	}

	/**
	 * Calls {@link #getAttributeTemplate(boolean)} and replaces the
	 * placeholders according to the {@link EAttributeHelpItemData}. Invokes
	 * {@link #renderEEnumLiterals(List)} in case the attribute type is an
	 * {@link EEnum}.
	 *
	 * @param attributes
	...
	public default String renderAttributes(List<EAttributeHelpItemData> attributes) {
		...
		if (!attributes.isEmpty()) {
			for (EAttributeHelpItemData attr : attributes) {
				text += this.getAttributeTemplate(attr.isEEnum()).replace("%NAME%", attr.getName())
						.replace("%TYPE%", attr.getDataType()).replace("%DESCRIPTION%", attr.getDocumentation())
						.replace("%EENUMVALUES%",
								attr.isEEnum() ? this.renderEEnumLiterals(attr.getEEnumLiterals()) : "")
	...
	public default String renderEEnumLiterals(List<EEnumLiteralHelpItemData> eEnumLiterals) {
		...
		for (EEnumLiteralHelpItemData lit : eEnumLiterals) {
			text += this.getEEnumLiteralTemplate().replace("%NAME%", lit.getName()).replace("%DESCRIPTION%",
					lit.getDocumentation());
		}
	...
	/**
	 * The default html template for rendering the help text of a
	 * non-containment {@link EReference}.
	 *
	 * @param refsAvailable
	 *            Whether to include the available references section.
	 * @return Contains placeholders for %NAME%, %TYPE%, %TAGS%, %DESCRIPTION%,
	 *         [%NCREFERENCEES%]
	 */
	public default String getNCReferenceTemplate(boolean refsAvailable) {
		return "<h3 class=\"heading ncreference-heading\">%NAME% \u00bb %TYPE%</h3>" + "%TAGS%"
				+ "<div class=\"ncreference-description description\">" + "%DESCRIPTION%" + "</div>"
				+ (refsAvailable
	...
	/**
	 * Calls {@link #getNCReferenceTemplate(boolean)} and replaces the
	 * placeholders according to the list of {@link EReferenceHelpItemData} of
	...
	public default String renderNCReferences(List<EReferenceHelpItemData> ncRefs) {
		...
		if (!ncRefs.isEmpty()) {
			for (EReferenceHelpItemData ncRef : ncRefs) {
				text += this.getNCReferenceTemplate(!ncRef.getChildData().isEmpty()).replace("%NAME%", ncRef.getName())
						.replace("%TYPE%", ncRef.getDataType()).replace("%DESCRIPTION%", ncRef.getDocumentation())
						.replace("%NCREFERENCEES%",
								!ncRef.getChildData().isEmpty() ? this.renderNCReferencees(ncRef.getChildData()) : "")
						.replace("%TAGS%", this.renderNCRefTags(ncRef));
			}
		}
		return text;
	}
	...
	/**
	 * Renders the tags of a reference help text. Calls
	 * {@link #getTagContainerTemplate()} and {@link #getTagTemplate()} and
	 * replaces the placeholder.
	 *
	 * @param ncRef
	 *            The non-containment reference to get the tags for.
	...
	public default String renderNCRefTags(EReferenceHelpItemData ncRef) {
		...
		if (ncRef.getEReference().isDerived()) {
			text += this.getTagTemplate().replace("%TYPE%", "ncref-tags").replace("%TEXT%", "derived");
		}
		if (!text.isEmpty()) {
			text = this.getTagContainerTemplate().replace("%TAGS%", text);
		}
	...
	/**
	 * Returns the template for including possible referenced objects
	 * (referencee) within the model in the help text.
	 *
	...
	public default String renderNCReferencees(List<EClassHelpItemData> childData) {
		...
		for (EClassHelpItemData target : childData) {
			text += this.getNCReferenceeTemplate().replace("%NAME%", target.getName()).replace("%DESCRIPTION%",
	 ...
	 *            Whether to include the creatable children section.
	...
	public default String getCReferenceTemplate(boolean childrenAvailable) {
		return "<h3 class=\"heading creference-heading\">%NAME% \u00bb %TYPE%</h3>"
				+ "<div class=\"creference-description description\">" + "%DESCRIPTION%" + "</div>"
				+ (childrenAvailable
	...
	/**
	 * Calls {@link #getCReferenceTemplate(boolean)} and replaces the
	 * placeholders.
	 *
	 * @param cRefs
	...
	public default String renderCReferences(List<EReferenceHelpItemData> cRefs) {
		...
		for (EReferenceHelpItemData cRef : cRefs) {
			...
			text += this.getCReferenceTemplate(!cRef.getChildData().isEmpty()).replace("%NAME%", cRef.getName())
					.replace("%TYPE%", cRef.getDataType()).replace("%DESCRIPTION%", cRef.getDocumentation())
					.replace("%CHILDREN%",
							!cRef.getChildData().isEmpty() ? this.renderCReferenceChildren(cRef.getChildData()) : "");
		}
		return text;
	}

	/**
	 * The default html template for providing the help text for creatable
	 * children of a containment reference.
	 *
	 * @param descriptionAvailable
	 *            Whether to include the description section of the creatable
	 *            child.
	 * @return Contains placeholders %NAME% [%DESCRIPTION%]
	 */
	public default String getCReferenceChildTemplate(boolean descriptionAvailable) {
		return "<h5 class=\"heading creference-child-heading\">\u00bb %NAME%</h5>" + (descriptionAvailable
				? "<div class=\"creference-child-description description\">%DESCRIPTION%</div>" : "");
	}

	/**
	 * Calls {@link #getCReferenceChildTemplate(boolean)} and replaces the
	 * placeholders.
	 *
	 * @param childData
	 * @return The rendered help text for the creatable children of a
	...
	public default String renderCReferenceChildren(List<EClassHelpItemData> childData) {
		...
		for (EClassHelpItemData child : childData) {
			text += this.getCReferenceChildTemplate(!child.getDocumentation().isEmpty())
					.replace("%NAME%", child.getName()).replace("%TYPE%", child.getDataType())
	 ...
	 * targets in the model that the eObject originates from. This
	 * implementation uses the default HelpItemFactory implementation.
	 *
	 * @param eObject
	 * @return all relevant {@link HelpItemDescription}
	 */
	public default HelpItemDescription getHelpItemDescription(EObject eObject) {
		return this.getHelpItemDescription(eObject, new HelpItemFactory() {
		});
	}
	 ...
	 * @param eObject
	 * @param factory
	 *            Specify a custom factory for the {@link HelpItemData} and
	 *            {@link HelpItemDescription} objects in order to provide custom
	 *            information to the {@link #render(HelpItemDescription)}
	 *            method.
	 * @return all relevant {@link HelpItemDescription}
	 */
			...
			for (IItemPropertyDescriptor itemPropertyDescriptor : propertyDescriptors) {
				...
				// Non-Containment References
					...
					if (ncRef.getEGenericType().getEClassifier() != null
							|| ncRef.getEGenericType().getERawType() != null) {
						helpItemDescription
								.addNonContainmentReferenceDescription(factory.createEReferenceHelpItemData(ncRef,
										itemPropertyDescriptor.getChoiceOfValues(eObject).stream()
												.filter(v -> v instanceof EObject).map(v -> (EObject) v)
			...
			for (Object cd : afid.getNewChildDescriptors(eObject, AgteleEcoreUtil.getEditingDomainFor(eObject), null)) {
				if (cd instanceof CommandParameter) {
					...
					List<EReferenceHelpItemData> matchingDesc = helpItemDescription.getContainmentReferenceDescription()
							.stream().filter(cRD -> cRD.getEReference().equals(feature)).collect(Collectors.toList());
					// if it doesn't create a new description for it
					if (matchingDesc.isEmpty()) {
						helpItemDescription.addContainmentReferenceDescription(factory.createEReferenceHelpItemData(
								(EReference) feature, Arrays.asList(((CommandParameter) cd).getEValue())));
					} else {
	...
	/**
	 * An overridable factory interface that creates HelpItems in order to be
	 * consumed by the rendering algorithm. Override the factory with custom
	 * methods and provide it to the
	 * {@link IEMFModelHelpItemProvider#getHelpItemDescription(EObject, HelpItemFactory)}
	 * in order to customize the content of the {@link HelpItemDescription} and
	 * {@link HelpItemData} objects that are used for the rendering of the help
	 * view.
	 *
	 * @author Baron
	 */
